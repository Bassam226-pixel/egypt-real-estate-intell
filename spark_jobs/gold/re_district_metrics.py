...
def re_district_metrics(spark: SparkSession) -> DataFrame:
    ...
    # Inner join: a yield is only meaningful where a district has both a sale read and
    # a rental read. That's exactly this dashboard-only mart's job -- re_sale_metrics
    # (the engine-eligible one) never sees this table or its rent/yield columns.
