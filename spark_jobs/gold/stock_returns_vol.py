...
# 126-day (6mo) lag would return all-nullptr -- see Layer_Tasks delta note.
...
def stock_returns_vol(spark: SparkSession) -> DataFrame:
    ...
    trailing = by_ticker.rowsBetween(-(VOL_WINDOW_DAYS - 1), 0)
    vol_90d = F.stddev("daily_return").over(trailing) * F.lit(TRADING_DAYS_PER_YEAR ** 0.5)
