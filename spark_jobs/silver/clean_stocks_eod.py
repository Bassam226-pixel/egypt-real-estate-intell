...
def clean_stocks_eod(spark: SparkSession) -> DataFrame:
    ...
    forward = by_ticker.rowsBetween(1, Window.unboundedFollowing)
    ...
    split_ratio = F.when(F.col("stock_splits") != 0, F.col("stock_splits")).otherwise(1.0)
    dividend_ratio = F.when(
        F.col("dividends").isNotNull() & (F.col("dividends") != 0),
        1.0 - F.col("dividends") / prev_close,
    ).otherwise(1.0)

    df = (
        df.withColumn("_log_factor", F.log(split_ratio) + F.log(dividend_ratio))
        .withColumn("_forward_factor", F.exp(F.sum("_log_factor").over(forward)))
