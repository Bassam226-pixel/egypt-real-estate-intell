...
def clean_fundamentals(spark: SparkSession) -> DataFrame:
    df = (
        io.read_csv(spark, "fundamentals_all.csv")
    ...
    # Sidesteps needing book value or shares outstanding, neither of which is in this feed.
    df = df.withColumn(
        "roe_proxy",
        F.when(
...
def clean_egx30_index(spark: SparkSession) -> DataFrame:
    ...
    return df.withColumn(
        "daily_return", F.col("close") / F.lag("close").over(by_date) - F.lit(1.0)
    ).select("trade_date", "open", "high", "low", "close", "volume", "daily_return")


def run() -> None:
    spark = get_spark_session("clean_fundamentals")
    ensure_namespaces(spark)
    io.write_table(io.add_metadata(clean_fundamentals(spark), "fundamentals_all.csv"), "silver", "fundamentals")
