...
def clean_re_rentals(spark: SparkSession) -> DataFrame:
    ...
    df = (
        ...
        .withColumn(
            "area_sqm",
            F.regexp_replace(F.regexp_extract(F.col("area"), LEADING_NUMBER, 1), ",", "").cast("double"),
        )
    )
    ...
    df = df.filter(
        ...
        & (F.col("area_sqm") > 0)
    ).withColumn("rent_per_sqm_egp", F.col("monthly_rent_egp") / F.col("area_sqm"))

    # Rentals only ever feed the dashboard's yield view, never the recommendation engine
    # (gold.re_district_metrics vs. gold.re_sale_metrics) -- tag that here so the
    # engine/dashboard boundary survives every downstream join, all the way to the
    # quality gate's boundary-rule check.
    df = df.withColumn("for_recommendation_use", F.lit(False))

    return df.dropDuplicates(["listing_id"]).select(
        "listing_id", "source", "title", "property_type",
        "governorate", "city", "district", "latitude", "longitude",
        "area_sqm", "monthly_rent_egp", "rent_per_sqm_egp",
        "bedrooms", "bathrooms", "furnished", "amenities", "agency",
        "posted_date", "scraped_at", "for_recommendation_use", "link",
    )
